...
const C = {
  bg: '#07070f', card: '#0f0f1e', dim: '#1a1a2e',
  border: 'rgba(255,255,255,0.07)', text: '#e2e8f0', muted: '#4a5568',
  sleep: '#818cf8', gym: '#34d399', money: '#fbbf24',
  habits: '#f472b6', mood: '#38bdf8',
}
...
const dayLabel = (iso) => ['Su','Mo','Tu','We','Th','Fr','Sa'][new Date(iso + 'T12:00:00').getDay()]
...
function computeStreak(data, filterFn) {
  ...
  for (let i = 0; i < 365; i++) {
    ...
    if (entry && filterFn(entry)) { streak++; d.setDate(d.getDate() - 1) }
    else break
  }
  return streak
}
...
function computeScore(sleep, gym, txns, habitData, moodLog) {
  ...
  const moneyScore = balance >= 0 ? Math.min(100, 70 + Math.log1p(balance) * 2) : Math.max(0, 50 + balance / 500)
  const habitsScore = days.map(d => {
  ...
  const moodScore = moodsW.length ? (moodsW.reduce((a, b) => a + b, 0) / moodsW.length / 5) * 100 : 50
...
export default function App() {
  ...
  const [habitData, setHabitData] = useState({ list: DEF_HABITS, logs: {} })
  const [moodLog, setMoodLog] = useState([])
  const [ready, setReady] = useState(false)

  useEffect(() => {
    ;(async () => {
      setSleep(await load('life:sleep') || [])
      setGym(await load('life:gym') || [])
      setTxns(await load('life:money') || [])
      setHabitData(await load('life:habits') || { list: DEF_HABITS, logs: {} })
      setMoodLog(await load('life:mood') || [])
      setReady(true)
    })()
  }, [])

  const updateSleep = e => { setSleep(e); save('life:sleep', e) }
  const updateGym = e => { setGym(e); save('life:gym', e) }
  const updateTxns = e => { setTxns(e); save('life:money', e) }
  const updateHabits = e => { setHabitData(e); save('life:habits', e) }
  const updateMood = e => { setMoodLog(e); save('life:mood', e) }

  const score = ready ? computeScore(sleep, gym, txns, habitData, moodLog) : 0
  const scoreColor = score >= 80 ? C.gym : score >= 60 ? C.money : score >= 40 ? C.habits : '#ef4444'

  return (
    <div style={{ background: C.bg, minHeight: '100vh', padding: '20px 18px 48px', fontFamily: "'DM Sans', system-ui, sans-serif", color: C.text, boxSizing: 'border-box' }}>
      <style>{`
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16, marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: C.muted, marginTop: 4 }}>Your life as data. What will you improve today?</div>
        </div>
        {ready && (
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: '14px 22px', textAlign: 'center', minWidth: 110 }}>
            <div style={{ fontSize: 10, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 6 }}>Life Score</div>
            <div style={{ fontSize: 44, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace", color: scoreColor, lineHeight: 1 }}>{score}</div>
      ...
      {ready && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(270px, 1fr))', gap: 14, marginTop: 14 }}>
            <HabitsCard data={habitData} update={updateHabits} />
            <MoodCard data={moodLog} update={updateMood} />
          </div>
        </>
      )}

      {!ready && (
        <div style={{ textAlign: 'center', color: C.muted, padding: '60px 0', fontFamily: 'monospace', fontSize: 13 }}>loading your data...</div>
      )}
    </div>
  )
}
...
function SleepCard({ data, update }) {
  ...
  const log = () => {
    const updated = [...data.filter(s => s.date !== today), { date: today, hours, quality }]
    update(updated); setOpen(false)
  }
    ...
    <div style={card}>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-end' }}>
        ...
        <div style={{ paddingBottom: 4 }}>
          ...
          {todayEntry && <KV k="quality" v={'★'.repeat(todayEntry.quality) + '☆'.repeat(5 - todayEntry.quality)} color={C.sleep} />}
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} barSize={16} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
            <Bar dataKey="hours" radius={[3, 3, 0, 0]}>
              {chartData.map((e, i) => <Cell key={i} fill={e.hours >= 7 ? C.sleep : e.hours > 0 ? C.sleep + '50' : C.dim} />)}
            </Bar>
            <XAxis dataKey="day" tick={{ fontSize: 10, fill: C.muted }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 12, color: C.text }} cursor={false} formatter={(v) => [v + 'h', 'sleep']} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      {open ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: C.muted }}>
            <span>Hours slept</span><Mono size={13} color={C.sleep}>{hours}h</Mono>
          </div>
          <input type="range" min="2" max="12" step="0.5" value={hours} onChange={e => setHours(+e.target.value)} style={{ accentColor: C.sleep, width: '100%' }} />
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            ...
            {[1, 2, 3, 4, 5].map(q => (
              <button key={q} onClick={() => setQuality(q)} style={{
                ...
                background: q <= quality ? C.sleep : C.dim, color: q <= quality ? '#fff' : C.muted,
              }}>{q}</button>
            ))}
          </div>
          <Row><Btn color={C.sleep} onClick={log}>{todayEntry ? 'Update' : 'Log sleep'}</Btn><Ghost onClick={() => setOpen(false)} /></Row>
        </div>
      ) : (
        <Btn color={C.sleep} onClick={() => setOpen(true)}>{todayEntry ? '↺ Update today' : '+ Log sleep'}</Btn>
      )}
    </div>
  )
}
...
function GymCard({ data, update }) {
  ...
  const log = () => {
    const updated = [...data.filter(g => g.date !== today), { date: today, type, mins }]
    update(updated); setOpen(false)
  }
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {TYPES.map(t => (
              <button key={t} onClick={() => setType(t)} style={{
                ...
                background: type === t ? C.gym : C.dim, color: type === t ? '#000' : C.muted,
              }}>{t}</button>
            ))}
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: C.muted }}>
            <span>Duration</span><Mono size={13} color={C.gym}>{mins} min</Mono>
          </div>
          <input type="range" min="15" max="120" step="5" value={mins} onChange={e => setMins(+e.target.value)} style={{ accentColor: C.gym, width: '100%' }} />
          <Row><Btn color={C.gym} onClick={log}>{todayEntry ? 'Update' : 'Log workout'}</Btn><Ghost onClick={() => setOpen(false)} /></Row>
        </div>
      ) : (
        <Btn color={C.gym} onClick={() => setOpen(true)}>{todayEntry ? `✓ ${todayEntry.type} done — update?` : '+ Log workout'}</Btn>
      )}
    </div>
  )
}
...
function MoneyCard({ data, update }) {
  ...
  const todaySpent = data.filter(t => t.date === today && t.type === 'expense').reduce((s, t) => s + t.amount, 0)
  const days = last7()
  ...
  const log = () => {
    ...
    setAmount(''); setLabel(''); setOpen(false)
  }
  ...
  return (
    <div style={card}>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-end' }}>
        <Mono size={28} color={balance >= 0 ? C.money : '#ef4444'}>{(balance >= 0 ? '+' : '-') + fmtAmt(balance)}</Mono>
        <div style={{ paddingBottom: 4 }}>
          <KV k="net balance" v="" />
      ...
      <div style={{ height: 72 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} barSize={12} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
            ...
            <XAxis dataKey="day" tick={{ fontSize: 10, fill: C.muted }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 12, color: C.text }} cursor={false}
      ...
      {open ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <input type="number" placeholder="Amount (₹)" value={amount} onChange={e => setAmount(e.target.value)}
            style={{ background: C.dim, border: `1px solid ${C.border}`, borderRadius: 8, padding: '9px 12px', color: C.text, fontSize: 13, outline: 'none', width: '100%' }} />
          <input type="text" placeholder="Label (e.g. Salary, Coffee...)" value={label} onChange={e => setLabel(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && log()}
            style={{ background: C.dim, border: `1px solid ${C.border}`, borderRadius: 8, padding: '9px 12px', color: C.text, fontSize: 13, outline: 'none', width: '100%' }} />
          <Row><Btn color={C.money} onClick={log}>Add entry</Btn><Ghost onClick={() => setOpen(false)} /></Row>
        </div>
      ) : (
...
function HabitsCard({ data, update }) {
  ...
  const removeHabit = (id) => update({ ...data, list: data.list.filter(h => h.id !== id) })
  ...
  const getStreak = (id) => {
    ...
    for (let i = 0; i < 365; i++) {
      const iso = d.toISOString().split('T')[0]
      if (data.logs[iso]?.includes(id)) { streak++; d.setDate(d.getDate() - 1) } else break
    }
    return streak
  }

  const weekCompletion = (id) => days.filter(d => data.logs[d]?.includes(id)).length
  ...
  return (
    <div style={card}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={() => setEditing(!editing)} style={{
            background: 'none', border: `1px solid ${C.border}`, borderRadius: 6, padding: '3px 9px',
      ...
      <div style={{ height: 4, background: C.dim, borderRadius: 2, overflow: 'hidden' }}>
        <div style={{ height: '100%', width: pct + '%', background: C.habits, borderRadius: 2, transition: 'width 0.4s ease' }} />
      </div>
          ...
          return (
            <div key={h.id} onClick={() => toggle(h.id)} style={{
              ...
              <div style={{
                ...
                alignItems: 'center', justifyContent: 'center', fontSize: 12, transition: 'all 0.15s',
                background: done ? C.habits : 'transparent', border: `2px solid ${done ? C.habits : C.muted}`,
                color: '#fff',
              }}>{done ? '✓' : ''}</div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ display: 'flex', gap: 2 }}>
                  {days.map((d, i) => (
                    <div key={i} style={{ width: 6, height: 6, borderRadius: 2, background: data.logs[d]?.includes(h.id) ? C.habits : C.border }} />
                  ))}
                </div>
                {streak > 0 && <span style={{ fontSize: 11, color: C.habits, fontFamily: 'monospace' }}>{streak}🔥</span>}
              </div>
              {editing && (
                <button onClick={e => { e.stopPropagation(); removeHabit(h.id) }} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: 18, padding: '0 2px', lineHeight: 1 }}>×</button>
              )}
            </div>
          )
        })}
      ...
      {editing && (
        <Row>
          <input value={newHabit} onChange={e => setNewHabit(e.target.value)} placeholder="New habit..."
            ...
            style={{ flex: 1, background: C.dim, border: `1px solid ${C.border}`, borderRadius: 8, padding: '8px 12px', color: C.text, fontSize: 13, outline: 'none' }} />
...
function MoodCard({ data, update }) {
  ...
  const avg = valid.length ? (valid.reduce((s, d) => s + d.score, 0) / valid.length).toFixed(1) : '—'
  ...
  return (
    <div style={card}>
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'center' }}>
        ...
        <div>
          <div style={{ fontSize: 16, fontWeight: 500, color: current ? C.text : C.muted }}>{current?.label || 'Not logged yet'}</div>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {MOODS.map(m => (
          <button key={m.score} onClick={() => logMood(m.score)} style={{
            flex: 1, padding: '10px 4px', borderRadius: 10, border: `1px solid ${todayEntry?.score === m.score ? C.mood : 'transparent'}`,
            background: todayEntry?.score === m.score ? C.mood + '25' : C.dim, cursor: 'pointer', fontSize: 22, transition: 'all 0.15s',
          }}>{m.emoji}</button>
        ))}
      </div>
      ...
      <div style={{ height: 72 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 8, right: 8, left: 8, bottom: 0 }}>
            <Line type="monotone" dataKey="score" stroke={C.mood} strokeWidth={2.5}
              dot={{ fill: C.mood, strokeWidth: 0, r: 3.5 }} connectNulls />
            <XAxis dataKey="day" tick={{ fontSize: 10, fill: C.muted }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, fontSize: 12, color: C.text }} cursor={false}
...
function SectionHead({ icon, label, color }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color }}>{label}</span>
    </div>
  )
}

function Mono({ size, color, children }) {
  return <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: size, fontWeight: 700, color, lineHeight: 1 }}>{children}</span>
}
